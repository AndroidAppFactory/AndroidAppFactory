...
class MessageTableModel extends BaseTableModel {
  ...
  static final String TABLE_CREATE_SQL =
    ...
    try {
      if (null != cursor && cursor.getCount() > 0) {
        ...
        MessageInfoItem item =
            JsonHelper.INSTANCE.fromJson(getStringByName(cursor, col_data), MessageInfoItem.class);
  ...
  static List<MessageInfoItem> getAllData(BaseDBHelper helper) {
    ...
    Cursor cursor =
        helper.queryInfo(
            TABLE_NAME, columns, selection, selectionArgs, groupBy, having, orderBy, limit);
    try {
      while (cursor.moveToNext()) {
        MessageInfoItem info =
            JsonHelper.INSTANCE.fromJson(getStringByName(cursor, col_data), MessageInfoItem.class);
