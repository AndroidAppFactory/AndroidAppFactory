...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SwipeButtonLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.SwipeButtonLayout_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeButtonLayout_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeButtonLayout_leftSwipe) {
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (Math.abs(gap) > 10 || Math.abs(getScrollX()) > 10) {//使屏蔽父布局滑动更加灵敏，
                        getParent().requestDisallowInterceptTouchEvent(true);
                    }
                    //侧滑菜单展开时，点击内容区域，关闭侧滑菜单。begin
