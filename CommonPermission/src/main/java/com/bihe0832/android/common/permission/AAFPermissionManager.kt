...
object AAFPermissionManager {
    ...
    fun initPermission() {
        ...
        PermissionManager.addPermissionGroupScene("", Manifest.permission.ACCESS_COARSE_LOCATION, "获取Wi-Fi名称、获取位置等")
    }
    ...
    fun checkSpecialPermission(activity: Activity, scene: String, canCancel: Boolean, tempPermissionList: List<String>, permissionsActivityClass: Class<out PermissionsActivity>, result: PermissionManager.OnPermissionResult?) {
        PermissionManager.checkPermission(activity, scene, canCancel, permissionsActivityClass, object : PermissionManager.OnPermissionResult {
            ...
            override fun onSuccess() {
                ...
                for (permission in tempPermissionList) {
                    if (!permissionExtraCheckIsOK(activity, permission)) {
                        isOK = false
                        try {
                            PermissionManager.startPermissionActivity(activity, scene, canCancel, permissionsActivityClass, tempPermissionList)
                        } catch (e: Exception) {
                            e.printStackTrace()
