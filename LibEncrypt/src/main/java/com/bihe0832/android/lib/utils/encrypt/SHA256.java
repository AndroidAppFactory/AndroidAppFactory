...
public class SHA256 {
    ...
    public static String getFilePartSHA256(String fileName, long start, long end) {
        return MessageDigestUtils.getFilePartDigestData(fileName, MESSAGE_DIGEST_TYPE_SHA256, start, end);
    }

    public static String getFilePartSHA256(File sourceFile, long start, long end) {
        return MessageDigestUtils.getFilePartDigestData(sourceFile, MESSAGE_DIGEST_TYPE_SHA256, start, end);
    }

    public static String getInputStreamSHA256(InputStream bis, long start, long end) {
        return MessageDigestUtils.getInputStreamPartDigestData(bis, MESSAGE_DIGEST_TYPE_SHA256, start, end);
    }
}
