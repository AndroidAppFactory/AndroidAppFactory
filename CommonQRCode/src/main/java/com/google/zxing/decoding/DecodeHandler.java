...
final class DecodeHandler extends Handler {
  ...
  public void handleMessage(Message message) {
    switch (message.what) {
      case MessageConstants.MSG_DECODE:
        //Log.d(TAG, "Got decode message");
        decode((byte[]) message.obj, message.arg1, message.arg2);
        break;
      case MessageConstants.MSG_QUIT:
  ...
  private void decode(byte[] data, int width, int height) {
    ...
    if (rawResult != null) {
      ...
      Message message = Message.obtain(activity.getHandler(), MessageConstants.MSG_DECODE_SUCCEEDED, rawResult);
      ...
      Message message = Message.obtain(activity.getHandler(), MessageConstants.MSG_DECODE_FAILED);
      message.sendToTarget();
    }
  }
