...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void addTab(final int position, CharSequence title, View tabView) {
        TextView tv_tab_title_text = (TextView) tabView.findViewById(R.id.tv_tab_title);
        ImageView tv_tab_title_image = (ImageView) tabView.findViewById(R.id.tv_tab_image);
        if (URLUtils.isHTTPUrl(title.toString())) {
            if (tv_tab_title_text != null) {
                tv_tab_title_text.setVisibility(View.GONE);

            }
            if (tv_tab_title_image != null) {
                tv_tab_title_image.setVisibility(View.VISIBLE);
                GlideExtKt.loadCircleCropImage(tv_tab_title_image, title.toString(), Color.TRANSPARENT,
                        Color.TRANSPARENT);
            }
        } else {
            if (tv_tab_title_text != null) {
                tv_tab_title_text.setVisibility(View.VISIBLE);
                if (title != null) {
                    tv_tab_title_text.setText(title);
                }
            }

            if (tv_tab_title_image != null) {
                tv_tab_title_image.setVisibility(View.GONE);
            }
        }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                ...
                tv_tab_title.setTextColor(isSelected ? mTextSelectColor : mTextUnselectColor);
                ...
                } else if (mTextBold == TEXT_BOLD_WHEN_SELECT && isSelected) {
                    tv_tab_title.setTypeface(null, Typeface.BOLD);
                } else {
            ...
            if (tv_tab_title_image != null) {
                tv_tab_title_image.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            }
        }
    }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                tab_title.setTextColor(isSelected ? mTextSelectColor : mTextUnselectColor);
                if (mTextBold == TEXT_BOLD_BOTH) {
                    tab_title.setTypeface(null, Typeface.BOLD);
                } else if (mTextBold == TEXT_BOLD_WHEN_SELECT && isSelected) {
                    tab_title.setTypeface(null, Typeface.BOLD);
                } else {
                    tab_title.setTypeface(null, Typeface.NORMAL);
                }
            }
        }
        if (null != mListener) {
    ...
    public void setIndicatorMargin(float indicatorMarginLeft, float indicatorMarginTop, float indicatorMarginRight,
    ...
    public TextView getTitleView(int tab) {
        return (TextView) getItemView(tab).findViewById(R.id.tv_tab_title);
    }

    public ImageView getImageView(int tab) {
        return (ImageView) getItemView(tab).findViewById(R.id.tv_tab_image);
    }
