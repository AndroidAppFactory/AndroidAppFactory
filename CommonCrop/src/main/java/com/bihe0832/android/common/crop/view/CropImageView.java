...
import com.bihe0832.android.common.crop.util.CubicEasing;
import com.bihe0832.android.common.crop.model.ImageState;
import com.bihe0832.android.lib.crop.jni.CropAction;
import com.bihe0832.android.lib.file.FileUtils;
import com.bihe0832.android.lib.log.ZLog;
import com.bihe0832.android.lib.media.image.ImageHeaderParser;
...
public class CropImageView extends TransformImageView {
     ...
     * Then creates and executes {@link CropAction} with proper parameters.
     */
    public void cropAndSaveImage(@NonNull final Bitmap.CompressFormat compressFormat, final int compressQuality,
        ...
        final RectF mCurrentImageRect = RectUtils.trapToRect(mCurrentImageCorners);
        ...
        ThreadManager.getInstance().start(new Runnable() {
            ...
            public void run() {
                ...
                if (mViewBitmap == null) {
                    new NullPointerException("ViewBitmap is null");
                } else if (mViewBitmap.isRecycled()) {
                    new NullPointerException("ViewBitmap is recycled");
                } else if (imageState.getCurrentImageRect().isEmpty()) {
                    new NullPointerException("CurrentImageRect is empty");
                }

                mCurrentScale = imageState.getCurrentScale();
                float resizeScale = resize(mViewBitmap, getImageInputPath(), getExifInfo());

                try {
                    crop(resizeScale, mCropRect, mCurrentImageRect, imageState.getCurrentAngle(), compressFormat,
                            getExifInfo(), compressQuality);
                    Uri uri = Uri.fromFile(new File(getImageOutputPath()));
                    cropCallback.onBitmapCropped(uri, cropOffsetX, cropOffsetY, mCroppedImageWidth,
                            mCroppedImageHeight);
                    return;
                } catch (Throwable throwable) {
                    throwable.printStackTrace();
                }
                cropCallback.onCropFailure(new Throwable());
            }
        });
    }
    ...
    private boolean crop(float resizeScale, RectF mCropRect, RectF mCurrentImageRect, float mCurrentAngle,
            Bitmap.CompressFormat mCompressFormat, ExifInfo mExifInfo, int mCompressQuality)
            throws IOException {
        ExifInterface originalExif = new ExifInterface(getImageInputPath());
        ...
        boolean shouldCrop = shouldCrop(mCroppedImageWidth, mCroppedImageHeight, mCurrentImageRect, mCurrentAngle);
        ZLog.i("Should crop: " + shouldCrop);

        if (shouldCrop) {
            boolean cropped = CropAction.crop(getImageInputPath(), getImageOutputPath(),
            ...
            if (cropped && mCompressFormat.equals(Bitmap.CompressFormat.JPEG)) {
                ImageHeaderParser.copyExif(originalExif, mCroppedImageWidth, mCroppedImageHeight, getImageOutputPath());
            }
            return cropped;
        } else {
            FileUtils.INSTANCE.copyFile(new File(getImageInputPath()), new File(getImageOutputPath()), false);
            return false;
        }
    }
     ...
     */
    private boolean shouldCrop(int width, int height, RectF mCurrentImageRect, float mCurrentAngle) {
        int pixelError = 1;
    ...
    private float resize(Bitmap mViewBitmap, String mImageInputPath, ExifInfo mExifInfo) {
    ...
    protected void processStyledAttributes(@NonNull TypedArray a) {
        float targetAspectRatioX = Math.abs(
                a.getFloat(R.styleable.CropView_aspect_ratio_x, DEFAULT_ASPECT_RATIO));
        float targetAspectRatioY = Math.abs(
                a.getFloat(R.styleable.CropView_aspect_ratio_y, DEFAULT_ASPECT_RATIO));
